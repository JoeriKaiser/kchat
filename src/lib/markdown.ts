...
marked.use({
	renderer: {
		code(token) {
			...
			return cleanHtml(`
				<div class="custom-code-block relative rounded-lg border border-timberwolf-600 bg-timberwolf-700 pale_dogwood-500 bg-isabelline-100 overflow-hidden">
				  <div class="flex items-center justify-between px-4 py-2 bg-pale_dogwood-500">
					...
					<pre class="m-0 p-0 overflow-auto text-sm leading-relaxed whitespace-pre-wrap break-words"><code class="hljs language-${language}">${highlightedHtml}</code></pre>
				</div>
			`);
		},
	},
...
export function parseAndSanitize(content: string): string {
	...
	const cleanHtml = DOMPurify.sanitize(dirtyHtml, {
		ADD_TAGS: ["button"],
		ADD_ATTR: ["data-code", "title", "aria-label"],
	});
	return cleanHtml;
}
